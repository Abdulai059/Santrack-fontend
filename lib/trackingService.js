...
export function startTracking(userId, options = {}) {
  ...
  async function push(position) {
    ...
    try {
      // 1. Update current location (user_locations table)
      const { error: currentError } = await supabase.from("user_locations").upsert(
        {
          user_id: userId,
          user_name: userName,
          user_role: userRole,
      ...
      // 2. Save to location history (location_history table)
      ...
      // Don't fail if history table doesn't exist yet
      if (historyError && historyError.code !== '42P01') {
        console.warn("[tracking] history save failed:", historyError.message);
      }
      
      console.log("✅ Location saved to database (current + history)");
